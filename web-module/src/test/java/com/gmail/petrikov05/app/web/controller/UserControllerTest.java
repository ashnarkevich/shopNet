...
import com.gmail.petrikov05.app.web.controler.UserController;
import com.gmail.petrikov05.app.web.controller.config.TestConfig;
...
import static com.gmail.petrikov05.app.web.controller.constant.TestUserConstant.VALID_ID;
import static com.gmail.petrikov05.app.web.controller.constant.TestUserConstant.VALID_IS_DELETED;
...
import static com.gmail.petrikov05.app.web.controller.constant.TestUserConstant.VALID_PAGES;
...
class UserControllerTest {
    ...
    @Test
    public void getUsers_returnValidUsers() throws Exception {
        PaginationWithEntitiesDTO<UserDTO> returnedPaginationWithEntitiesDTO = getValidPaginationWithEntitiesDTO();
        when(userService.getUsersByPage(anyInt())).thenReturn(returnedPaginationWithEntitiesDTO);
        mockMvc.perform(
                get("/users")
    ...
    public void deleteUsersWithEmptyIds_returnBadRequest() throws Exception {
        mockMvc.perform(
                ...
                .andExpect(redirectedUrl("/users?page=" + VALID_PAGE))
                .andExpect(flash().attribute("message", "didn't check"));
    }
    ...
    @Test
    public void changePassword_redirectUsersWithPage() throws Exception, UserExistenceException, AdministratorChangingException {
        UserDTO returnedUserDTO = getValidUserDTO();
        when(userService.changePassword(anyLong())).thenReturn(returnedUserDTO);
        mockMvc.perform(
    ...
    @Test
    public void changePassword_returnBAdRequest() throws Exception {
        mockMvc.perform(
                get("/users/a/changePass")
                        .param("page", String.valueOf(VALID_PAGE))
        ).andExpect(status().isBadRequest());
    }

    @Test
    public void changePasswordWithInvalidPage_returnBAdRequest() throws Exception {
        mockMvc.perform(
                get("/users/2/changePass")
                        .contentType(MediaType.TEXT_HTML)
        ).andExpect(status().isBadRequest());
    }

    @Test
    public void changePassword_callBusinessLogic() throws Exception, UserExistenceException, AdministratorChangingException {
        UserDTO returnedUserDTO = getValidUserDTO();
        when(userService.changePassword(anyLong())).thenReturn(returnedUserDTO);
        mockMvc.perform(
                get("/users/2/changePass")
        ...
        verify(userService, times(1)).changePassword(anyLong());
    }

    @Test
    public void changePassword_redirectUsersWithMessage() throws Exception, UserExistenceException, AdministratorChangingException {
        UserDTO returnedUserDTO = getValidUserDTO();
        when(userService.changePassword(anyLong())).thenReturn(returnedUserDTO);
        mockMvc.perform(
                get("/users/2/changePass")
                ...
                .andExpect(flash().attribute("message", "A password changed. A new password sent on the email: test@test.test"));
    }

    @Test
    public void changePassword_returnUserExistenceException() throws Exception, UserExistenceException, AdministratorChangingException {
        when(userService.changePassword(anyLong())).thenThrow(new UserExistenceException("user not found"));
        mockMvc.perform(
                get("/users/2/changePass")
                        .param("page", String.valueOf(VALID_PAGE))
        ).andExpect(status().is3xxRedirection())
    ...
    @Test
    public void changePasswordSuperAdministrator_returnAdmin() throws Exception, UserExistenceException, AdministratorChangingException {
        when(userService.changePassword(anyLong()))
                .thenThrow(new AdministratorChangingException("super administrator doesn't delete"));
        mockMvc.perform(
                get("/users/2/changePass")
                        .param("page", String.valueOf(VALID_PAGE))
        ).andExpect(status().is3xxRedirection())
                .andExpect(flash().attribute("message", "A password didn't change super administrator."));
    }
    ...
    /* add new user */
    @Test
    public void addUser_returnStatusRedirect() throws Exception {
        UserDTO returnedUser = getValidUserDTO();
        try {
